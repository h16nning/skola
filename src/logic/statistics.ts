    ...
    if (existingStats) {
        db.statistics.put({
            ...
            time: {
                ...
                forReview:
                    existingStats.time.forReview + statistics.time.forReview,
                forLearning:
                    existingStats.time.forLearning +
                    statistics.time.forLearning,
            },
            cards: {
                [State.New]:
                    existingStats.cards[State.New] +
                    statistics.cards[State.New],
                [State.Review]:
                    existingStats.cards[State.Review] +
                    statistics.cards[State.Review],
                [State.Learning]:
            ...
            ratingsList: existingStats.ratingsList.concat(
                statistics.ratingsList,
            ),
        });
    } else {
