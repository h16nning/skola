import { Button, Card, FileButton, Group, Modal, Stack, Text, TextInput } from "@mantine/core";
...
async function importCards(fileText: string | null, deck: Deck | undefined, cardSeparator: string, questionAnswerSeperator: string) {
    if(!fileText || !deck) {
        return;
    }
...
export default function ImportCardsModal({
  ...
  const [questionAnswerSeperator, setQuestionAnswerSeperator] = useState<string>("\t");
  ...
  return (
    <Modal opened={opened} onClose={() => {setOpened(false); setFile(null)}} title="Import Cards">
      <Stack align="start">
        <Card withBorder shadow="xs" w="100%" sx={{display: "flex", placeContent: "center"}}>
        {! file ? <FileButton
          onChange={(f) => {
            setFile(f);
          }}
        ...
        </FileButton> : <Group position="apart" align="center" w="100%"><Text fz="sm" fw={500}>{file.name}</Text> <Button variant="default" onClick={() => setFile(null)}>Remove File</Button></Group>}
        </Card>
        <TextInput
        ...
        />
        <Button sx={{alignSelf: "end"}}onClick={() => importCards(fileText, deck, cardSeparator, questionAnswerSeperator)} disabled={!file || !fileText || !deck}>Import Cards</Button>
      </Stack>
    </Modal>
  );
}
