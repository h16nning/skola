...
let showNotificationFn: ((notification: { title: string; message: string; type: 'success' | 'error' | 'info'; autoClose?: number | false }) => string) | null = null;

export function setNotificationHandler(handler: typeof showNotificationFn) {
  showNotificationFn = handler;
}

function showNotification(title: string, message: string, type: 'success' | 'error' | 'info', autoClose?: number | false) {
...
export function successfullyDeleted(type: 'card' | 'deck' | 'note') {
  const titles = { card: 'Card Deleted', deck: 'Deck Deleted', note: 'Note Deleted' };
  return showNotification(
    titles[type],
